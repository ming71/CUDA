...
__global__ void ROIPoolForward(const int nthreads, const scalar_t *bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    // 一个thread计算输出7*7的一个像素，也就是一个bin的pooling坐标
    ...
    // 计算得到当前计算bin的位置，由于采用了floor和ceil明显是有量化误差的
    ...
    // 多了个argmax因为这里采用的是maxpool，所以需要记录最大值的位置（索引），便于反向传播计算，如果是均值池化就不用
    int max_idx = -1;
    bottom_data += (roi_batch_ind * channels + c) * height * width;

    // Define an empty pooling region to be zero
    // max_val变量采用冒泡逐个查找比较存储下最大的值
